    ...
    class giris
    {
        ...
        public void girisYap(string kullaniciAdi, string kullaniciSifre, DateTime tarih)
        {
            ...
            try
            {
                ...
                if(kulAdi_Oku.Read())                   //eğer kullanıcı adı doğru girildiyse
                {
                    ...
                    if(loginPw_Oku.Read())
                    {
                        ...
                        SqlCommand dateUpdate = new SqlCommand("update kullaniciBilgileri set girisTarihi=@tarih where kullaniciAdi = @kuladi AND kullaniciSifre = @kulsifre",db.baglanti); //tarih güncelleniyor
